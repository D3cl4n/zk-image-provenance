...
2) Edit `png_editor.py` to point to an RGB PNG file in `\images`
3) Run `python png_editor.py`
4) Edit `main.rs` to the edited image path
5) Run `cargo build --release` from the top-level directory
6) Run `cargo run --release` from the top-level directory
